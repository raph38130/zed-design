...
{
	int i;
	for (i=0;i<=63 ;i++) int_seq[i] = 0x00000000;

	//for (i=0;i<=DELAY ;i++) Xil_Out32(OLED_BASE+(64), 1);
	//for (i=0;i<=DELAY ;i++) Xil_Out32(OLED_BASE+(64), 0);
}
...
int print_char( char char_seq , unsigned int page, unsigned int position)
{
	...
	if (position > 15)
		...
		int p = (position-(position%4))+offset;
		int_seq[p] |=  ascii_value;
		Xil_Out32(OLED_BASE+(p),int_seq[p]);
